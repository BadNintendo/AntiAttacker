#C:/Users/elite/AppData/Local/Programs/Python/Python313/python.exe "c:/Users/elite/OneDrive/Desktop/Work Space/AntiPP.py"
import time
import socket
import select
import json
...
PROTECTED_PORTS = [80, 443, 21, 22, 25, 53, 110, 143, 3306, 8080]  # Add/remove as needed
...
def start_server_on_port(host, port, log_file):
    tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    tcp_socket.bind((host, port))
    tcp_socket.listen(5)
    logging.info(f"TCP server listening on {host}:{port}")
    styled_print(f"TCP server listening on {host}:{port}")

    udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_socket.bind((host, port))
    logging.info(f"UDP server listening on {host}:{port}")
    styled_print(f"UDP server listening on {host}:{port}")

    threading.Thread(target=watch_sockets, args=([tcp_socket, udp_socket],), daemon=True).start()
    threading.Thread(target=handle_data, args=(tcp_socket, udp_socket), daemon=True).start()

# 🚀 Server bootstrap
def combined_server(log_file='server_log.txt'):
    setup_logging(log_file)
    host = get_local_ip()
    logging.info("Multi-port server starting...")
    styled_print("Multi-port server starting...")

    for port in PROTECTED_PORTS:
        try:
            start_server_on_port(host, port, log_file)
        except Exception as e:
            logging.error(f"Failed to start on port {port}: {e}")
            styled_print(f"Failed to start on port {port}: {e}")

    # 🧱 Block main thread to keep servers alive
    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        styled_print("Server shutdown requested. Exiting...")
